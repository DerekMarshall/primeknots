...
#endif
#ifndef M0B_PROVIDER_HASH
#define M0B_PROVIDER_HASH "M0B_PROVIDER_HASH-undefined"
#endif

namespace at::murm {

const char* ss_generator_hash() { return SS_GENERATOR_HASH; }
const char* m0b_provider_hash() { return M0B_PROVIDER_HASH; }
...
void write_ss_run(const std::string& path, const SSRun& run) {
    ...
    os << "# ss_empirical v" << run.format_version
       ...
       << "# ap_provider " << run.ap_provider << "\n";
    if (run.ap_provider == "m0b") os << "# provider_hash " << run.provider_hash << "\n";
    os << "# X_confirm " << run.X_confirm << "\n"
       << "# du " << run.du << "\n"
       << "# tol " << run.tol << "\n"
...
SSRun read_ss_run(const std::string& path) {
    ...
    while (std::getline(is, line)) {
        ...
        if (line[0] == '#') {
            ...
            else if (key == "ap_provider") ms >> r.ap_provider;      // absent ⇒ "fast" (legacy)
            else if (key == "provider_hash") ms >> r.provider_hash;
...
void write_ss_partials(const std::string& path, const SSPartials& P,
    ...
    os << "# ss_partials v" << meta.format_version
       ...
       << "# ap_provider " << meta.ap_provider << "\n";
    if (meta.ap_provider == "m0b") os << "# provider_hash " << meta.provider_hash << "\n";
    os << "# X " << meta.X << "\n"
       << "# du " << meta.du << "\n"
       << "# NB " << meta.NB << "\n"
...
SSPartials read_ss_partials(const std::string& path, SSPartialsMeta& out_meta,
    ...
    while (std::getline(is, line)) {
        ...
        if (line[0] == '#') {
            ...
            else if (key == "ap_provider") ms >> m.ap_provider;      // absent ⇒ "fast" (legacy)
            else if (key == "provider_hash") ms >> m.provider_hash;
