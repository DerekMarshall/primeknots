...
an estimate and is labelled as one throughout; it is not yet computed (input archive pending).
...
## 2. Claude web — ESTIMATED (labelled estimate; not yet computed)

**Status: PENDING.** The Claude web export archive has not been supplied; no web token figures
are computed here. No estimate is stated until the archive is parsed. The method is fixed in
advance:

- **Method.** Parse the export conversation JSON; per message, **characters ÷ 4** as the token
  proxy. Tag each conversation into one of: orchestration; referee rounds; cold-clone review;
  blind transcription. Report per-tag and total, input and output separately where the export
  distinguishes role.
- **Honesty band.** Every figure carries **±30%** (the chars/4 proxy is not a tokenizer count;
  the band is stated, not implied).
- **Separation.** These estimates are never combined with the §1 measured Claude Code tokens
...
de-duplicated by message ID, cross-checked against ccusage 20.0.17. §2 estimate pending its input
archive; method and ±30% band fixed. §3 recorded from PR-1.md, m0b-pinning.md,
