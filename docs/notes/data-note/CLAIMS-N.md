...
| N2-3 | Statistic (1): `D̂(u_b) = (Δu·\|fam\|)⁻¹ Σ_{(E,p):p/N∈bin_b} (u_mid·lnN/N)·ε·a_p`; `Δu=0.025`, 40 bins | SS25 eq (1); src/murm/ss_empirical.h:17–19; data/m5/ss_partials_x65536.txt header (`du 0.025`, `NB 40`); **eq (1) now rendered in the source's exact form (`p∈(C₁N(E),C₂N(E))` prime, exponent inside the log `log(N(E)^{(C₁+C₂)/2})`); `D̂` is our binned estimator of it; char-level re-checked vs SS25 eq (1) p.2 (2026-07-18)** |
