...
Status: **§1 + §2 rows complete; §3–§6 [PENDING]** (drafted after §1+§2 review, per R4).

## §1 — Introduction

| ID | Claim | Source |
|----|-------|--------|
| N1-1 | Murmurations = oscillating correlation between `a_p(E)` and `p`; discovered by HLOP (2022), since established in several settings | HLOP arXiv:2204.10140 (abstract, p.3 "never reported"); docs/explainers/E1-murmurations.md (E1-1, E1-21) |
...
| N2-10 | Commit-before-run pre-registration; PR-1 ladder rule (`fa4e35a`), PR-2 threshold (`100a13f`), PR-3 leakage estimator (`21060a0` prereg / `0d21b62` measured) | RESEARCH-M §7; docs/preregistered/PR-1.md, PR-2.md, PR-3.md; git commit hashes as cited |
...
*(Rows added as §3 Results, §4 Mechanism constraints, §5 Reproducibility, §6 Discussion are
drafted — per R4, only after §1+§2 are refereed. Each results/constraint number must trace to a
committed `ss_x*.txt` run, an emitted JSON, or a named test output before its sentence lands.)*
