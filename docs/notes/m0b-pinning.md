...
**Ladder status (2026-07-16):** **anchors ✓** (§6 PARI-adjudicated, incl. the A2 twist path) ·
**x16 full twin ✓** (`twin_m0b_vs_charsum_x16`: 112,266,653 (curve,prime) a_p reproduced EXACTLY,
0 mismatches — incl. the 235,275-pair p≤229 charsum-fallback band; 3044.7 s single-thread vs the
36860 s / 12-thread charsum generation ⇒ **~145× less CPU** for the identical grid, O(p^{1/4}) vs
O(p)). **Pending:** x18 tail-weighted sample, `oracle_ellap` spot → PRODUCTION-CAPABLE; **x17** (on
the box post-cache) is the trailing data-note gate. E3 three-algorithm clause licensed + shipped
(x16); its two-platform clause stays held for x17.
