...
grids passing green would be **silent under-coverage** (the clean-skip lesson, in cache form).

**The x17 twin is the strongest single test the repo has (GREEN 2026-07-17).** It is a
**cross-ALGORITHM** twin: the 2¹⁷ reference a_p cache is generated by the frozen charsum referee
(`ap_fast`, spot-checked by `ap_charsum`), and Shanks–Mestre (M0b) must reproduce it EXACTLY.
Both the reference cache and the M0b run execute on **FreeBSD** — i.e. **same platform**. That is
NOT a limitation and NOT hidden: a_p is a **platform-independent integer** (no libm — proven
byte-identical laptop↔FreeBSD), so the platform axis is moot for a_p and a two-platform a_p twin
would verify nothing extra. (The separate *platform*-robustness fact — that the float-weighted
*binned statistic* reproduces across macOS↔FreeBSD — is demonstrated at 2¹⁶ in the data-note
reproducibility section; it is a different claim from this integer twin.) **Honest scope:
cross-algorithm, same-platform, a_p platform-moot — NOT a "two-platform twin."**
...
**Ladder status (2026-07-17):** **anchors ✓** (§6 PARI-adjudicated, incl. the A2 twist path) ·
**x16 full twin ✓** (`twin_m0b_vs_charsum_x16`: 112,266,653 (curve,prime) a_p reproduced EXACTLY,
0 mismatches — incl. the 235,275-pair p≤229 charsum-fallback band; **~145× less CPU** than the
36860 s / 12-thread charsum generation for the identical grid, O(p^{1/4}) vs O(p)) · **x17 full
twin ✓** (`twin_m0b_vs_charsum_x17`: **385,792,535** (curve,prime) a_p reproduced EXACTLY, 0
mismatches — incl. the 421,321-pair p≤229 fallback band; **276.5 s wall on 48 threads**, vs the
...
## 7a. Reference a_p cache — generator contract (2b design pins, 2026-07-15)

The reference caches (twins 1 & 2) are produced by a dedicated frozen-referee generator
(`at ap-cache`), pinned as follows:

- **P1 — grid identity is part of the contract.** The cache header records the grid **definition**,
  not merely a value hash: the **ne_cache checksum**, the **per-curve prime rule** — exactly
  `{p prime : 3 < p ≤ N(E) and p ∤ (4A³+27B²)}` (the good-prime domain of the statistic,
  `ss_empirical.cpp:64–68`) — the **curve-set descriptor** (`height_family(X)`, conductor-ascending)
  and the **chunk layout**. The twin-side REQUIRE is pinned in §7.
- **P2 — circularity firewall (STANDING RULE).** The generator uses the **frozen referee only**
  (`ap_fast`, spot-checked by `ap_charsum` per existing discipline) and has **NO code path to
  `ap_shanks_mestre`, ever.** The header carries **`algorithm: charsum_referee`** provenance, so a
  reference cache and any future M0b-produced cache are distinguishable **by inspection.** *A
  reference cache is never regenerated by the algorithm it referees, regardless of speed temptation.*
- **P3 — disk (measured, not guessed).** ~1.3 GB local approved (gitignored, R3). Realized sizes and
  generation wall times are recorded here as each cache lands (the keep-full-vs-sample decision at
  2¹⁸ wants measured growth): **2¹⁶ — 224.5 MB, 36860 s (10.2 h, laptop 12-thread), clean (1 attempt);
  2¹⁷ — generating on FreeBSD 48-core (moved off-laptop; the cache is a platform-independent integer
  artifact — proven byte-identical cross-platform).**

Engineering (per the `e8396cb` conductor-sorted chunked pattern): conductor-ascending, chunked and
resumable, checkpoint on the real filesystem; **byte-identity of the sorted-vs-canonical layout
asserted on a scratch range before the long run.**
