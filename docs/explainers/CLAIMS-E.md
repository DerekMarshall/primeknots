...
| E1-13 | Example-1 sizes (isogeny-class representatives): 4328, 5194 in [7500,10⁴]; 8536, 1380 in [5000,10⁴]; via LMFDB | HLOP Example 1 + eq (1.1) "(representatives for the isogeny classes of)", p.3 |
...
| E1-16 | The limit is a murmuration density `M(y)`, `y` a prime-over-scale ratio | Z25 p.2 (M_k(y) def) |
| E1-17 | Our plots put the prime-over-scale ratio, not `p`, on the horizontal axis | viz/data/murmuration_curve.json (y_axis "p/N2"); sawin_..._murmuration.json (u=p/N) |
| E1-18 | Square-free vs all levels give slightly different densities (key properties shared) | Z25 p.2, footnote 2 |
| E1-19 | The EC murmuration is "very sensitive to ordering by conductor, and disappears almost entirely" under naive height / j-invariant / discriminant | Z25 p.5 (verbatim) |
| E1-20 | SS isolate a subtler surviving height-ordered density (Conjecture 1); our M4/PR-1 measures that fainter signal | SS25 Conjecture 1; Z25 p.5; docs/preregistered/PR-1.md |
| E1-35 | Three distinct variables: Z25 `y:=P/X` (level scale), SS25 `u=p/N(E)` (per-curve conductor), M1 `p/N₂` (band-top); the denominator is part of the statistic | m3-pinning §P5 ("Let y := P/X … NOT P/N"); SS25 eq (1) (u=p/N); viz/data/murmuration_curve.json (p/N2) |
| E1-36 | The height-ordered EC density is a separate conjecture; its tail is where an initial comparison flagged an apparent deviation, later traced to our own eq (2) transcription (ERRATA #28) | SS25 Conjecture 1; ERRATA #28; docs/preregistered/PR-1.md postscript |
...
| E1-27 | EC murmurations "not explained by these results, as they constitute a very sparse subset of weight 2 modular forms"; the family average does not determine the sparse EC subfamily | Z25 p.5 (verbatim) |
...
| E1-32 | M4+PR-1 test SS Conjecture 1 over a four-rung ladder (\|fam\| 1048/5042/9014/15936 at 10⁴/2¹⁶/2¹⁷/2¹⁸); an apparent trough displacement was traced to an eq (2) transcription error (ERRATA #28); corrected, ALL THREE invariants (hump, zero, trough) within τ, trough a small resolved residual ≈0.012; PR-1 Rung-3 verdict SUPERSEDED (gated against the corrupted target, void as pronounced) | sawin_..._murmuration.json, ss_x_extension_murmuration.json (verdict "superseded"); ERRATA #28; PR-1.md; tests `prereg_ss_x_extension`, `prereg_ss_x17_confirmation` |
| E1-33 | Height-ordered replication succeeds within τ after ERRATA #28 (the "trough deviation" was the transcription error); PR-2 (analytic-rank-2 split — empirical trough unmoved) and PR-3 (root-number leakage — bounded) stand as measurements from the investigation; the small resolved residual's decay with X is the open PR-5 question | ERRATA #28; PR-1.md; docs/preregistered/PR-2.md, PR-3.md; PR-5 (named future work) |
