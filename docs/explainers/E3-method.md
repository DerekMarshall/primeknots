<!-- EXPLAINER E3. Linked from README; content accuracy (rule 1) is referee-owned.
...
expected number appeared. The point of a *verification* project is to make that failure mode
**detectable and costly** — caught by a rule and recorded — rather than merely discouraged.
Five devices do the work.
...
- **PR-1** committed, *before* extending the computation to a larger scale, a single decision
  ...
  persists," and committed the interpretation fork before reading the larger-scale output. The
  rule was applied verbatim and returned the "persists" branch — but *against a density target a
  later audit round found corrupted* (a transcription error in our reading of eq (2), ERRATA
  #28). Because the verdict was gated on that target, it is **superseded, void as pronounced** —
  recorded, not quietly re-run against the corrected target it was never registered against.
  Pre-registration and the errata ledger together: the rule was followed honestly, and when its
  target failed, the verdict was voided in public rather than salvaged. <!-- claim:E3-11 -->
...
source to high precision or generated by a checked-in script. <!-- claim:E3-15 --> And **#28** is
the reason for a second standing rule — that a transcription check terminates at the *source
artifact* (the paper PDF), never at an internal quote: a swapped-product misreading of eq (2),
carried from a pinning note into the code *and into a fresh re-implementation built from the same
quote*, so three internally-consistent implementations agreed on the wrong density. Only the
authors' published figure caught it. (It also re-inverted #26: that entry held the peek's apparent
trough *agreement* was overturned into a "persistent deviation" — but #28 shows the *deviation*
was the artifact, and the corrected comparison agrees; the protocol breach #26 records still
stands, its punchline does not.) <!-- claim:E3-16 -->
