...
## 6. Stage M4 — Sawin–Sutherland height ordering [hard anchor]

[SS25] proves an explicit murmuration density for elliptic curves ordered by **naive height** (not conductor), via Voronoi-type summation. Replication: build the height-ordered family from Weierstrass coefficients directly (no conductor needed for the ordering — largely oracle-free), average per their statistic, verify against their density. **[PIN TO SOURCE]:** their exact height definition, family weighting, and density formula. This rung matters doubly: it is a proven anchor, *and* it establishes the ordering axis (conductor vs height) as a controlled, tested pipeline parameter — which is precisely the axis the research mode probes.
