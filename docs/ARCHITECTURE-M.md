...
Named so the tests can target them: sign/off-by-one in a_p (global, silent → source-quoted anchor + model invariance); family-definition drift from the paper (soft anchors make this invisible → definitions quoted, counts certified); cache staleness (→ generator-hash refusal); pattern-fitting (→ prereg contract, holdout disjointness check, looks ledger); tolerance shopping (→ tolerances justified in logs before runs, changes to a tolerance are log events); oracle-provenance leakage into computed claims (→ §2 declaration + CI grep); **post-hoc change to a pre-declared design** (tolerance / null / threshold), all modes (→ **commit the design before the run it governs**, replication mode included — uncommitted pre-registration is unenforceable, ERRATA #19; a later change is a logged deviation postscript, never a silent edit; RESEARCH-M §7); **source-extraction garble** (a mis-read of a paper's rendered equation, e.g. a `pdftotext` artifact → "verbatim" means *eyes on the rendered source page*, not the text layer; caught at M2 pinning on LOP eq. (1.3)).
